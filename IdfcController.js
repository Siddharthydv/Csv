...
const newCsv = require('./newCsv');
 function IdfcController(inputFile,outputFile)
{
    ...
    .pipe(csv({
        mapHeaders: ({ header, index }) => {
            ...
            const headers = [ "Transaction Details","Date", "Debit", "Credit",];
    ...
    .on('data', (row) => {
        ...
        if(row.Date=="International Transactions")
        {
            // console.log("yooooooo")
            zone="International"
            Person="";
        }
        else if(row.Date==="Rahul"|| row.Date==="Ritu")
            Person=row.Date;
        if(Person!=="")
        {
        ...
        if(zone=="Domestic")
            {
                ...
                // Extract organization name (remaining words joined)
                const orgName = words.join(" ");
                row.Place=city;
            }   
            else{
    ...
    .on('end', () => {
        ...
        const res=results.filter(row=>row.Debit!==''|| row.Credit!==''&&(row.Credit!=='Domestic Transactions'&&row.Credit!=='International Transactions'))
        res.sort((a, b) => {
