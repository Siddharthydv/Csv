...
const newCsv = require('./newCsv');
 function AxisController(inputFile,outputFile)
{
    ...
    .pipe(csv({
        mapHeaders: ({ header, index }) => {
            ...
            const headers = ["Date", "Debit", "Credit", "Transaction Details"];
    ...
    .on('data', (row) => {
        ...
        //
        if(row.Credit=="International Transactions")
        {
            zone="International"
            Person="";
        }
        else if(row.Credit==="Rahul"|| row.Credit==="Ritu")
            Person=row.Credit;
        if(Person!=="")
        {
